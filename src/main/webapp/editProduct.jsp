...
<title>Sửa Sản Phẩm - Kachi-Kun Shop</title>
		...
		<div class="main-content">
			<div class="page-header">
				<h2>
					...
					<span style="font-size: 16px; color: #888; font-weight: 400;">#${product.id}</span>
				</h2>
					...
					<div class="form-row">
								...
								<select id="categoryId" name="categoryId" class="form-control">
									...
									<c:forEach var="category" items="${categories}">
										<option value="${category.id}"
											<c:if test="${product.category != null and product.category.id == category.id}">selected</c:if>>
											${category.name}
										</option>
									</c:forEach>
								</select>
								...
								<select id="brandId" name="brandId" class="form-control">
									...
									<c:forEach var="brand" items="${brands}">
										<option value="${brand.id}"
											<c:if test="${product.brand != null and product.brand.id == brand.id}">selected</c:if>>
											${brand.name}
										</option>
									</c:forEach>
								</select>
						...
						<div class="form-col">
							...
							<div class="form-group">
								...
								<div class="image-preview-box" id="previewBox" style="${not empty product.image ? 'border-color:#2d7e7e;' : ''}">
									<c:choose>
										<c:when test="${not empty product.image}">
											<img id="imgPreview" src="images/${product.image}" alt="Preview" style="display:block;">
										</c:when>
										<c:otherwise>
							...
							<div class="form-group">
								...
								<input type="text" id="size" name="size" class="form-control"
									placeholder="Ví dụ: 120x60x40mm" value="${product.size}">
							</div>
						</div>
					</div>
