...
public class RegisterServlet extends HttpServlet {
    ...
    private void handleSendOtp(HttpServletRequest request, HttpServletResponse response)
        ...
        request.setAttribute("reg_sentAt", sentAt);
        request.getRequestDispatcher("login.jsp").forward(request, response);
    }
    ...
    private void handleVerifyOtp(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!storedOtp.equals(inputOtp)) {
            ...
            request.setAttribute("reg_sentAt", session.getAttribute("reg_sentAt"));
            request.getRequestDispatcher("login.jsp").forward(request, response);
            return;
        }
    ...
    private void handleResendOtp(HttpServletRequest request, HttpServletResponse response)
        ...
        String otp = generateOtp();
        long sentAt = System.currentTimeMillis();
        long expiry = sentAt + 5 * 60 * 1000;
        session.setAttribute("reg_otp", otp);
        session.setAttribute("reg_expiry", expiry);
        session.setAttribute("reg_sentAt", sentAt);

        boolean sent = EmailService.sendEmail(email,
                "Mã xác nhận đăng ký - Kachi-Kun Shop (Gửi lại)",
        ...
        if (sent) {
            request.setAttribute("resendSuccess", "Đã gửi lại mã OTP mới đến " + email);
        } else {
            request.setAttribute("registerError", "Không thể gửi lại email. Vui lòng thử lại sau!");
        }
