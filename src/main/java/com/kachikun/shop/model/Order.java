...
public class Order {
    ...
    public String getStatusColor() {
        if (STATUS_UNPAID.equals(status)) return "text-orange-700 bg-orange-100";
        if (STATUS_PAID.equals(status)) return "text-indigo-700 bg-indigo-100";
    ...
    public int getTimelineStep() {
        ...
        if (STATUS_UNPAID.equals(status)) return 1;
        if (STATUS_PAID.equals(status)) return 1;
    ...
    public boolean isCancellable() {
        return STATUS_PENDING.equals(status) || STATUS_UNPAID.equals(status);
    }
}